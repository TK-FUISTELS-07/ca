                    ...
                    ${temporadas.map((temp, idx) => `
                        <div class="tk-capitulos-temporada ${idx === 0 ? 'tk-visible' : 'tk-oculta'}" data-temporada="${temp.id}">
                            ${temp.capitulos.map(cap => `
                                <div class="tk-capitulo-card" data-ocean-fixed="true" onclick="reproducirCapituloDirectoDesdeCard(event, '${cap.href}', '${temp.id}', '${cap.numero.replace(/'/g, "\\'")}', '${cap.titulo.replace(/'/g, "\\'")}')">
