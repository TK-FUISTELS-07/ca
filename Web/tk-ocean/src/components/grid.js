...
const upgradeImage = window.ApiService ? window.ApiService.upgradeImage : (file) => file;

function upgradeImagesInContainer(container) {
    container.querySelectorAll('img[src*="tmdb.org"]').forEach(img => {
...
function createCard(item, onClick) {
    ...
    img.src = item.image ? upgradeImage(item.image) : 'https://via.placeholder.com/300x450?text=sin+imagen';
    img.alt = item.title;
    img.loading = 'lazy';
