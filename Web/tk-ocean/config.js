...
// CONFIGURACIÓN DEL DOMINIO - CAMBIAR AQUÍ PARA CAMBIAR DE DOMINIO
const PRODUCTION_DOMAIN = 'ocean-mirror.serveirc.com';

const IS_LOCALHOST = window.location.hostname === 'localhost' ||
    window.location.hostname === '127.0.0.1';

// Puertos (usados solo en desarrollo local)
const PORTS = {
...
const SERVER_IP = IS_LOCALHOST ? 'localhost' : PRODUCTION_DOMAIN;
const SERVER_PROTOCOL = IS_LOCALHOST ? 'http' : 'https';

// URLs relativas para producción (usando proxy de Nginx)
const API_BASE_URL = IS_LOCALHOST ? `${SERVER_PROTOCOL}://${SERVER_IP}:5000` : '/api';
const API_WATCHER_URL = IS_LOCALHOST ? `${SERVER_PROTOCOL}://${SERVER_IP}:5002` : '/watcher';
const LOCALHOST_BRIDGE = IS_LOCALHOST ? `${SERVER_PROTOCOL}://${SERVER_IP}:5000/` : '/api/';
...
const CONFIG = {
    ...
    // Funciones helper
    utils: {
        // Generar URL completa del Bridge
        getBridgeUrl: (path) => {
            if (path.startsWith('http')) return path;
            return `${API_BASE_URL}${path.startsWith('/') ? '' : '/'}${path}`;
        },

        // Generar URL del stream proxy
        getStreamUrl: (url) => {
            if (!url) return '';
            if (url.includes(':5002/stream_proxy')) return url;
            return `${API_WATCHER_URL}/stream_proxy?url=${encodeURIComponent(url)}`;
        },
        ...
        filterEmbedLinks: (links) => {
            ...
            const filtered = links.filter(url => {
                ...
                if (url.includes(':5002/stream_proxy')) return true;
        ...
        extractPathFromMirror: (url) => {
            const bridgeBase = API_BASE_URL + '/';
            const localhostBase = LOCALHOST_BRIDGE;
            return url.replace(bridgeBase, '').replace(localhostBase, '');
        },
