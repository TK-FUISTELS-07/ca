...
async function fetchPage(route) {
    // 1. Obtener la base configurada (ej: https://ocean-mirror.serveirc.com/mirror/)
    let baseUrl = window.CONFIG ? window.CONFIG.API_BASE : (window.API_BASE_URL + '/');

    // 2. Limpiar la ruta de entrada eliminando prefijos no deseados
    let cleanedRoute = route.replace(/^\/tk-ocean\//, '').replace(/^\/detalle\//, '');

    // 3. Limpiar la ruta para evitar barras dobles
    const cleanRoute = cleanedRoute.startsWith('/') ? cleanedRoute.slice(1) : cleanedRoute;

    // 4. Construir URL final (Ej: https://.../mirror/peliculas)
    const finalUrl = `${baseUrl}${cleanRoute}`;

    console.log('[API] Solicitando contenido a:', finalUrl);

    try {
        const res = await fetch(finalUrl, {
            // Importante: No forzar headers si el mirror es simple, 
            // pero 'Accept' ayuda a que Go sepa qué entregar
            headers: { 'Accept': 'text/html,application/xhtml+xml' },
            mode: 'cors'
        });

        if (!res.ok) {
            throw new Error(`Error ${res.status}: No se encontró la ruta en el Mirror`);
        }

        return await res.text();
    } catch (error) {
        console.error('[API] Error crítico de conexión:', error);
        throw error; // Es importante relanzar el error para que la UI lo maneje
    }
}
