...
async function fetchPage(route) {
    const cleanSlug = CONFIG.utils.extractPathFromMirror(route);
    const finalUrl = `${CONFIG.API_BASE}${cleanSlug}`;

    const res = await fetch(finalUrl, { mode: 'cors' });
    if (!res.ok) {
        console.error("Error en Bridge:", res.status);
        throw new Error("El servidor Bridge no devolvió una respuesta válida");
    }
