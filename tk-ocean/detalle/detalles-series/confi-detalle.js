...
function corregirEnlacesMirror(tipoActual, slugActual) {
    ...
    const allLinks = document.querySelectorAll('a[href*="192.168.0.23:5000"], a[href*="localhost:5000"], a[href*="127.0.0.1:5000"]');
...
window.reproducirCapituloDirectoDesdeCard = async function (event, path, tempIndex, episodeCode, title) {

    event.preventDefault();
    event.stopPropagation();

    // LIMPIAR EL PATH ANTES DE HACER NADA
    const slugLimpio = CONFIG.utils.extractPathFromMirror(path);

    // Ahora hacer la petición con el slug que Go sí entiende
    const fullSlug = slugLimpio;
    // Extraer tipo, serieSlug, temporada y capítulo del slug
    const parts = fullSlug.split('/');
    const tipo = parts[0]; // 'serie' o 'anime'
    ...
    if (tempIndexInParts > -1 && capIndexInParts > -1) {
        serieSlug = parts.slice(1, tempIndexInParts).join('/');
        temporada = parseInt(parts[tempIndexInParts + 1]);
        capitulo = parseInt(parts[capIndexInParts + 1]);
    } else {
        // Fallback si no tiene temporada/capitulo
        serieSlug = parts.slice(1).join('/');
