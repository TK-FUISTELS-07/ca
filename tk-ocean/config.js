...
const CONFIG = {
    ...
    utils: {
        ...
        extractPathFromMirror: (url) => {
            ...
            // Si hay ?slug=, toma el valor decodificado
            if (clean.includes('?slug=')) {
                return decodeURIComponent(clean.split('?slug=')[1]);
            }
            // Quita prefijos de la web
            clean = clean.replace('/tk-ocean/detalle/detalles-series/capitulos/', '')
                .replace('/tk-ocean/detalle/', '')
                .replace('/tk-ocean/', '')
                .replace('/detalle/', '');
            // Quita barras iniciales y decodifica
